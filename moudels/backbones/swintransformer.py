...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = x[:, 1::2, 0::2, 0::2, :]  # B D H/2 W/2 C
        x4 = x[:, 1::2, 1::2, 0::2, :]  # B D H/2 W/2 C
        x5 = x[:, 0::2, 1::2, 1::2, :]  # B D H/2 W/2 C
...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(d_dim)
        x = x.view(B, D*2**self.d_dim, H, W, C)
        x = rearrange(x, 'b d h w (p1 p2 c)-> b d (h p1) (w p2) c', p1=self.dim_scale, p2=self.dim_scale, c=C // 4)
...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            ...
            x = self.norm(x)
            # print("norm",x.shape,)
            x = x.transpose(1, 2).contiguous().view(-1, self.embed_dim, Dd, Wh, Ww)
            # print("teran",x.shape)
        # print("embed",x.shape)
        return x
...
class SwinTransformerSys3D_encoder(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, pretrained=None,
        ...
        for i in range(self.num_layers):
            # expand = nn.Linear(dim,dim*4*)
            # if i>3:
            #     expand = nn.Linear(embed_dim*2**i, 4 * 8 * embed_dim*2**i, bias=False)
            # else:
            #     expand = nn.Linear(embed_dim*2**i, 4 * 16 * embed_dim*2**i, bias=False)
            expand = nn.Linear(embed_dim * 2 ** i, 4 * 16 * embed_dim * 2 ** i, bias=False)
    ...
    def forward(self, x):
        ...
        # print(x.shape,"dddd",len(x_downsample))
        for i,expand in enumerate(self.expand):
            # print(x_downsample[i].shape, "xdxdxd")
            B_xd,C_xd,D_xd,H_xd,W_xd = x_downsample[i].shape
            xd = x_downsample[i].flatten(2).transpose(1, 2).contiguous()
            # print(xd.shape, "xdxdxd22")
            xd = expand(xd)
            # print(xd.shape, "xdxdxd33")
            b,l,c = xd.shape
            # print(xd.shape, "xdxdxd33")
            xd = xd.view(B,D_xd,H_xd,W_xd,c).contiguous()
            # print(xd.shape, "xdxdxd33")

            xd = rearrange(xd, 'b d h w (p1 p2 p3 c)-> b (d p1) (h p2) (w p3) c', p1=4, p2=4,
                          p3=4,
                          c=c // (4 ** 3))
            # print(xd.shape, "xdxdxd33ddddddddddddd")
            x_downsample[i] = xd.permute(0, 4, 1, 2,3).contiguous()
            # x_downsample[i] = conv_layer(x_downsample[i])
            # print(x_downsample[i].shape, "xdxdxd")
...
if __name__ == "__main__":
    ...
    swin = SwinTransformerSys3D_encoder(
        pretrained=None,
        img_size=(128, 128, 128),
        patch_size=(4, 4, 4),
        in_chans=32,
    ...
    x = swin(rga)
    flops =FlopCountAnalysis(swin,rga)
    # print(y.shape)
    print("flops:",flops.total())
    print(parameter_count_table(swin))
    #
    # net = SwinTransformerBlock3D(
    #             dim=1024,
    #             num_heads=num_heads,
    #             window_size=window_size,
    #             shift_size=(0, 0, 0) if (i % 2 == 0) else self.shift_size,
